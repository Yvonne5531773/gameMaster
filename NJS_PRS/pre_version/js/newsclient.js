...
var NewsClient = {
	...
	init: function(){
		...
		if(NewsClient.from == 'jinli'){
			//猎户广告请求
			$.ajax({
				//url:'http://g.news.liebao.cn/js/lh.json',
...
var NewsDetail = {
	...
	init:function(){
		...
		if(NewsClient.from == 'jinli'){
			...
			//猎户广告请求
			$.ajax({
				//url:'http://g.news.liebao.cn/js/lh2.json',
				url:'http://rtb.mobad.ijinshan.com/b/?',
				dataType: 'json',
				type:'GET',
				data:{
	...
	getDataContent: function(id, productid){
		$.ajax({
			...
			url:'http://g.m.liebao.cn/news/detail?scenario=0x00000101&lan=zh_CN&osv=5.1.1&appv=3.29.0&app_lan=zh_CN&ch=10000000&pid=17&action=0x3af&uuid='+Cookie.get('Lb_uuid')+'&pf=android&net=wifi&v=4&mnc=&ctype=0x26B&display=0xCF&contentid='+id+'&brand=google&mcc=&nmnc=&nmcc=&aid=f3e439a8082bd706&model=Nexus+5',
			dataType: 'json',
			type:'GET',
			success: function(result) {
	...
	renderHtml: function(data){
		...
		$(".openPic").on("click",function(){
			if(IsWeixin){
				window.location.href="http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster";
				return;
			}else if(IsAndroid){
			...
			setTimeout("window.location.href='http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster';",500);
		})
		//iframe过渡
...
//		});
		...
		//更多按钮
//		if(channel == ''){
//			$(".detail-read").attr('href','http://g.news.liebao.cn/?f='+from+'#channel=news');
//		}else if(channel == 'top'){
//			$(".detail-read").attr('href','http://g.news.liebao.cn/top.html?f='+from);
//		}else if(channel.indexOf('group') >= 0){
//			var albumid = NewsDetail.getQueryValue('albumid','&');
//			$(".detail-read").attr('href','http://g.news.liebao.cn/group.html?albumid='+albumid+'&f='+from);
//		}else{
//			$(".detail-read").attr('href','http://g.news.liebao.cn/?f='+from+'#channel='+channel);
//		}
		...
		var appCover = 'http://g.news.liebao.cn/images/jrkb.png';
		...
		if(URL.indexOf('cmcleanresult') != -1 || URL.indexOf('cmshare') != -1 || URL.indexOf('cmhometab') != -1 || URL.indexOf('cmscreensaver') != -1){
			...
			if(!cmshare){
				...
				$('#tuiApp').on('click', function(){
					NewsDetail.cmDownLoad('com.cmcm.gamemaster', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', '猎豹浏览器', 'http://g.news.liebao.cn/images/cmb_cover.png');
				...
				if(URL.indexOf('cmcleanresult') != -1){
					if (!IsLiebaoFast) {
					//猎豹CM推送
						NewsDetail.tuiApp(['http://g.news.liebao.cn/images/cmb_cover.png','猎豹浏览器','独家新闻，个性呈现','打开', '猎豹浏览器：专属于你的头条新闻','http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', '猎豹浏览器CM清理结果顶部下载', '猎豹浏览器CM清理结果底部下载']);
				...
				if(IsCM && CMObj.hasOwnProperty('checkInstall')) {
					var isInstall = JSON.parse(CMObj.checkInstall('["com.cmcm.gamemaster"]'));
					if(isInstall['com.cmcm.gamemaster']){
				...
				$('.i-more').on('click', function(){
					if(typeof android != 'undefined' && android.hasOwnProperty('share')) {
						android.share(6,'http://g.news.liebao.cn/detail.html?f=cmshare&newsid='+NewsDetail.newsid, '', '', 'http://g.news.liebao.cn/images/jrkb.png');
					}
					_czc.push(['_trackEvent', 'CM新闻分享', '分享量', '', '', '']);
				});
				$('.i-weibo').attr('data-cmd', '').on('click', function(){
					if(typeof android != 'undefined' && android.hasOwnProperty('share')) {
						android.share(4,'http://g.news.liebao.cn/detail.html?f=cmshare&newsid='+NewsDetail.newsid, '', '', '');
					}
					_czc.push(['_trackEvent', 'CM新闻分享', '分享量', '', '', '']);
				});
				$('.i-qzone').attr('data-cmd', '').on('click', function(e){
					e.preventDefault();
					if(typeof android != 'undefined' && android.hasOwnProperty('share')) {
						android.share(7,'http://g.news.liebao.cn/detail.html?f=cmshare&newsid='+NewsDetail.newsid, '', '', '');
					}
					_czc.push(['_trackEvent', 'CM新闻分享', '分享量', '', '', '']);
				});
		...
		if(formApp == 'cms' || formApp == 'cmsnotification'){
			var cms_version = 0;
			var cms_apkjson = '{"pkgName":"com.cmcm.gamemaster","verCode":"30000666","size":"1965305","url":"http://dl.liebao.cn/android/tg/kbrowser_cm_toutiao.apk","appName":"猎豹浏览器"}';
			...
			//判断是否安装并调起
			if(typeof window.NativeCall != 'undefined'){
				var isInstall = JSON.parse(window.NativeCall.checkInstall('["com.cmcm.gamemaster"]'))['com.cmcm.gamemaster'];
				if(isInstall){
					//调起APP
		...
		if(formApp == 'lbcm'){
			if (!IsLiebaoFast) {
				NewsDetail.tuiApp(['http://g.news.liebao.cn/images/cmb_cover.png','猎豹浏览器','独家新闻，个性呈现','', '猎豹浏览器：专属于你的头条新闻','http://dl.liebao.cn/android/cm/cheetah_cm_share1.apk', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', '猎豹浏览器CM推送顶部下载', '猎豹浏览器CM推送底部下载']);
				_czc.push(['_trackEvent', '猎豹浏览器CM推送', '展现量', '', '', '']);
			}
		}
...
//		if(IsAndroid){
			...
			if(formApp == 'lbfx'){
				if (!IsLiebaoFast) {
					NewsDetail.tuiApp(['http://g.news.liebao.cn/images/cmb_cover.png','游戏超人','玩游戏必备利器，游戏超人带你飞','', '猎豹浏览器：专属于你的头条新闻','http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', '浏览器顶部下载', '浏览器底部下载']);
				...
				//非定制下载
				if (!IsLiebaoFast) {
					NewsDetail.tuiApp(['http://g.news.liebao.cn/images/cmb_cover.png','游戏超人','玩游戏必备利器，游戏超人带你飞','', '猎豹浏览器：专属于你的头条新闻','http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', '非定制顶部下载','非定制底部下载']);
...
//			Cookie.set('Lb_detailad',1);
			...
			if($('#cy-latest-list-wrapper').length && !isFlag){
				...
				if(IsIOS){
					...
					if($('#cy-latest-list-wrapper article').length >= 2){
						...
						NewsDetail.tuiApps(['.changyan-mores', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', 'http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster', (formApp == 'lbfx' ? '浏览器评论' : '评论'), '查看更多展现']);
					}
				}
				isFlag = true;
	...
	hotNews:function(type, contentid){
		...
		$.ajax({
			...
			url:'http://g.m.liebao.cn/news/recommend?scenario=0x00000505&lan=zh_CN&osv=5.1.1&appv=3.29.0&app_lan=zh_CN&ch=10000000&pid=17&action=0x3af&uuid='+Cookie.get('Lb_uuid')+'&pf=android&net=wifi&v=4&mnc=&ctype=0x01&display=0xCF&contentid='+contentid+'&brand=google&mcc=&nmnc=&nmcc=&aid=f3e439a8082bd706&model=Nexus+5',
			dataType: 'json',
			type:'GET',
			success: function(result) {
				...
				var html = [],
					...
					$(".swiper-slideNews,.open_liebao").on("click",function(){
						...
						var href="http://a.app.qq.com/o/simple.jsp?pkgname=com.cmcm.gamemaster";
	...
	renderHtml: function(data){
		...
		if(data.videos.length){
			...
			for(var i=0; i<item; i++){
				html.push('<li><a href="http://g.news.liebao.cn/video.html?newsid='+data.videos[i].aid+'"><span><img src="'+data.videos[i].img+'"/><i>'+new Date(parseInt(data.videos[i].duration) * 1000).format("mm:ss")+'<em>'+data.videos[i].reads+'</em></i></span><p>'+data.videos[i].title+'</p></a></li>');
			}
...
var NewsTopic = {
	...
	getDataContent: function(id){
		$.ajax({
			url:'http://g.m.liebao.cn/news/album?contentid='+id+'&model=MI+3W&mcc=460&osv=4.4.4&ctype=0x1EB&pid=17&display=0xCF&aid=53b9b52f336c1023&net=wifi&nmnc=00&mode=2&appv=3.27.2&v=3&pf=web&scenario=0x00000101&lan=en_US&action=0x0f&brand=Xiaomi&nmcc=460&mnc=00&ch=10000000&lastupdatetime=0&offset=',
			dataType: 'json',
			type:'GET',
			jsonpCallback:"callback",
			success: function(result){
	...
	renderListHTML: function(result){
		...
		for(var i=0, l=dataList.length; i<l; i++){
			switch(dataList[i].display){
				case '0x01':
					//无图模式
					html.push('<dl class="i-style02"><a href="http://g.news.liebao.cn/detail.html?newsid='+dataList[i].contentid+'&f=topic"><dt>'+dataList[i].title+'</dt><dd><p class="i-infos"><span class="i-source">'+dataList[i].source+'</span></p></dd></a></dl>');
					break;
				case '0x02':
				case '0x04':
					//小图模式
					html.push('<dl class="i-style01"><a href="http://g.news.liebao.cn/detail.html?newsid='+dataList[i].contentid+'&f=topic" class="clearfix"><dt><span class="imgbox-wrap"><img class="imglazyload imgbox" src="'+defaultImg+'" osrc="'+dataList[i].images[0]+'"></span></dt><dd><p>'+dataList[i].title+'</p><p class="i-infos"><span class="i-source">'+dataList[i].source+'</span></p></dd></a></dl>');
					break;
				case '0x08':
					//大图模式
					html.push('<dl class="i-style00"><a href="http://g.news.liebao.cn/detail.html?newsid='+dataList[i].contentid+'&f=topic"><dd><span class="imgbox-wrap"><img class="imglazyload imgbox" src="'+defaultImg+'" osrc="'+dataList[i].images[0]+'"><i>'+dataList[i].title+'</i></span></dd></a></dl>');
					break;
				default:
					break;
			}
...
var SetStat = {
	init: function(uuid, url, module, channel, from){
		$.ajax({
			url:'http://g.news.liebao.cn/api/proxy.php',
			dataType: 'json',
			type:'POST',
